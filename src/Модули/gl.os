...
Функция ОтправитьРезультатВнешнейПроверки(Знач Запрос, Знач РезультатПроверки) Экспорт

	IDПроекта = ПроектВнешнейПроверки(Запрос);
	IIDМержРеквеста = МержРеквестВнешнейПроверки(Запрос);

	Результат = Новый Структура;
	Результат.Вставить("id", IDПроекта);
	Результат.Вставить("merge_request_iid", IIDМержРеквеста);
	Результат.Вставить("sha", Запрос["object_attributes"]["last_commit"]["id"]);
	Результат.Вставить("external_status_check_id", Запрос["external_approval_rule"]["id"]);
	...
	Запрос = СтрШаблон("%1/projects/%2/merge_requests/%3/status_check_responses", КореньAPI(), IDПроекта, IIDМержРеквеста);
	Возврат КоннекторHTTP.Post(Запрос, Результат, , ПараметрыСоединения()).Json();
КонецФункции

Функция СтатусВнешнейПроверки(Знач Статус)
...
Функция ПередвинутьТег(Знач ИмяТега, Знач Проект, Знач Коммит) Экспорт
	...
	Исключение
		Лог.Отладка("Не смог удалить тег %1 в проекте %2:
